        ...
        function updateLunarDateInfo() {
            ...
            document.getElementById('lunarMonth').value = Math.abs(month);
...
function genYearGrid(year, solarBase, lunarBase){
    ...
    grid.innerHTML = '';
    for(let m=1; m<=12; m++){
        const s = calculateFinalSum(solarBase + year + m + getSoulOffset());
        const l = calculateFinalSum(lunarBase + year + m + getSoulOffset());
    ...
    for(let r=0; r<6 && d<=daysInMonth; r++){
        ...
        for(let c=0; c<7; c++){
            if(r===0 && c<firstWeekday || d>daysInMonth){
                ...
                const s = calculateFinalSum(solarBase + year + month + d + getSoulOffset());
                const l = calculateFinalSum(lunarBase + year + month + d + getSoulOffset());
...
function updateFlowBase(){
    ...
    FLOW_BASE.lunar = (ry + 1911) + lm + ld; // 改為國曆出生年 + 農月 + 農日
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(sel){
    sel.addEventListener('change', ()=>{
      // 重新觸發主要計算並刷新年/月視圖
      if(typeof calculateNumerology==='function'){ calculateNumerology(); }
      if(FLOW_BASE.solar!==null){
        genYearGrid(FLOW_BASE.year, FLOW_BASE.solar, FLOW_BASE.lunar);
        document.getElementById('monthFlowCalendar')?.classList.add('hidden');
      }
    });
